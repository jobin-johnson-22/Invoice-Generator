...
import { addInvoice, editInvoice } from '../redux/invoiceSlice/action';
...
class InvoiceForm extends React.Component {
  constructor(props) {
    ...
    const edit = props.invoiceState.invoices.filter(
      (inv) => inv.invoiceNumber === invoice
    );
    this.state = edit[0] ? edit[0] : this.props.invoiceState.invoices[0];
    if (this.addTo)
      this.state.invoiceNumber =
        props.invoiceState.invoices[
          Number(props.invoiceState.invoices.length) - 1
        ].invoiceNumber;
...
export default connect(mapStateToProps, { addInvoice, editInvoice })(
  InvoiceForm
);
